...
export function WorkshopGrid({
  ...
  const paginatedWorkshop = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return workshop.slice(startIndex, endIndex);
  }, [workshop, currentPage]);

  const totalPages = Math.ceil(workshop.length / itemsPerPage);

  // Reset to page 1 when workshop change
  useEffect(() => {
    setCurrentPage(1);
  }, [workshop.length]);
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-3">
                    ...
                    {isAdmin && (
                      <div className="relative">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="workshop-menu-button text-white transition-opacity"
                          onClick={() =>
                            onOpenMenuChange(
        ...
        {!isPageLoading && workshop.length === 0 && (
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  <h3 className="text-lg font-medium">
                    No se encontraron herramientas del Taller
                  </h3>
      ...
      {workshop.length > 0 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            ...
            {Math.min(currentPage * itemsPerPage, workshop.length)} de{" "}
            {workshop.length} herramientas
          </div>
          <Pagination
